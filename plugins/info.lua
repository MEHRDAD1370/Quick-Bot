...
local function setrank(msg, name, value) -- setrank function
  ...
  if hash then
	...
	return send_msg('channel#id'..msg.to.id, '„ﬁ«„ ò«—»— ('..name..') »Â '..value..'  €ÌÌ— œ«œÂ ‘œ ', ok_cb,  true)
  end
end
   ...
   Username = '‰œ«—œ'
    ...
    if not value then
	 if result.id == tonumber(MKH) then
	   text = text..'„ﬁ«„ :»«»«Ì „‰ - „œÌ— ò· —»«  (Executive Admin) \n\n'
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  ...
  local text = '‰«„ : '..(result.from.first_name or '')..'\n›«„Ì· :'..(result.from.last_name or '----')..'\n'
               ..'ÌÊ“—‰Ì„ : '..Username..'\n'
               ..'«ÌœÌ : '..result.from.peer_id..'\n\n'
	local hash = '„ﬁ«„:'..result.to.id..':variables'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = text..'‘„«—Â : '..msg.from.phone..'\n\n'
   local text = text..'·Ì‰ò ò«—»— : telegram.me/'..msg.from.username..'\n\n'
   local hash = 'rank:variables'
	if hash then
