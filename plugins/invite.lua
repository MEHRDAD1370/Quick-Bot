...
--vardump(result)
  local user = 'user#id'..result.peer_id
	...
	if is_banned(result.id, extra.chatid) then 
        send_large_msg(chat, 'íæÒÑ Èä ÔÏå ÇÓÊ')
        send_large_msg(channel, 'íæÒÑ Èä ÔÏå ÇÓÊ')
	elseif is_gbanned(result.id) then
	    send_large_msg(chat, 'íæÒÑ áæÈÇá Èä ÔÏå ÇÓÊ')
		send_large_msg(channel, 'íæÒÑ áæÈÇá Èä ÔÏå ÇÓÊ')
	else    
...
function run(msg, matches)
  ...
  if not is_admin1(msg) then -- For admins only !
		return 'İŞØ ÇÏãíä åÇ Çã˜Çä ÏÚæÊ ÑÇ ÏÇÑäÏ'
  end
